...
Summarizes text after payment authorization. The first unsigned request receives a `402 Payment Required` response with a request-bound authorization v2 `paymentContext`. The signed retry includes the EIP-712 signature and claimed payer headers.
