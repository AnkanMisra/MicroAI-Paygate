...
- `requestHash`: SHA-256 hash of the exact serialized request body bytes.

## Receipt Fields

Receipts include:

- `version`: `2.0` for request-bound authorization receipts. Clients may continue to verify stored legacy `1.0` receipts using the v1 serialization.
- payment payer, recipient, amount, token, chain ID, nonce, and signed authorization timestamp.
- service endpoint.
