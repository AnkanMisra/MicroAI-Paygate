...
// serialized in alphabetical order. This ensures consistent signatures.
// Non-determinism issues only affect map types, which we don't use here.
func signReceipt(receipt Receipt) (*SignedReceipt, error) {
	...
	// For structs, json.Marshal always outputs fields in alphabetical order
