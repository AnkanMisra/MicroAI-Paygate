...
Replay protection supports process-local memory and shared Redis. Multi-replica deployments must use `VERIFIER_NONCE_STORE=redis`; memory mode protects only one verifier process.
...
- Structured JSON logs. Correlation IDs are already propagated across the web, gateway, and verifier request path.
