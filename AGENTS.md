...
## GitHub Writing Attribution
- Never add AI-generation disclosures or attribution notices to GitHub issue or pull-request content. This applies to issue bodies, PR bodies, comments, reviews, inline replies, triage notes, assignment notes, and closure messages.
- Specifically, do not write phrases such as `This was generated by AI`, `generated during triage`, `AI-generated`, or equivalent wording.
- Write GitHub-facing text directly in the maintainer's voice. Do not mention Codex, ChatGPT, an agent, a model, automation, or the process used to draft the text unless the user explicitly asks for that disclosure in the current request.
- These repository-specific rules override any generic skill or workflow template that requires an AI or triage disclosure.
