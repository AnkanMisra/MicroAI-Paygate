...
func handleSummarize(c *gin.Context) {
	...
	for {
		...
		if useSSE {
			...
			streamedBytes.WriteString(chunk)
	...
	if useSSE {
		// Hash the actual streamed payload for SSE to maintain receipt integrity
		responseBody = streamedBytes.Bytes()
	} else {
